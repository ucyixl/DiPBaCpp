...
void initialiseDiPBaC(baseGeneratorType& rndGenerator,
	...
	params.setSizes(nSubjects,nCovariates,nFixedEffects,nCategoriesY,nPredictSubjects,nCategories);
	...
	if(includeResponse){
		...
		for(unsigned int c=0;c<maxNClusters;c++){
			for (unsigned int k=0;k<nCategoriesY;k++){
				...
				params.theta(c,k,-2.0+4.0*unifRand());
			}
		}
		...
		for(unsigned int j=0;j<nFixedEffects;j++){
			for (unsigned int k=0;k<nCategoriesY;k++){
				...
				params.beta(j,k,-2.0+4.0*unifRand());
			}
		}
...
string storeLogFileData(const diPBaCOptions& options,
	...
	if(dataset.nCategoriesY()>1){
		tmpStr << "NumberOfCategoriesY: " << endl;
		tmpStr << dataset.nCategoriesY() << endl;
	}
